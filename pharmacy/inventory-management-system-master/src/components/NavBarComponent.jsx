...
export default function NavBarComponent() {
  ...
  const doLogout = ()=>{ auth.logout(); navigate('/login'); }
                    ...
                    <>
                      ...
                      >
                        <Tooltip title="account settings">
                          <Avatar sx={{ width: 32, height: 32 }}>{auth.user.name?.[0]||'U'}</Avatar>
                        </Tooltip>
                      </IconButton>
                      <Typography fontFamily={"Inter"}>{auth.user.name} ({auth.user.role})</Typography>
                    </>
                  ) : (
